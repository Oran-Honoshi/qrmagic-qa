      ...
      <div className="max-w-6xl mx-auto space-y-6">
        ...
        >
          ...
          <div className="relative z-10 flex items-center justify-between p-6 gap-4 flex-wrap">
            <div>
              <p className="text-xs font-semibold text-[#4A5568] uppercase tracking-wider mb-1">Welcome back</p>
              <h2 className="text-xl font-black text-[#F0F4F8] tracking-tight mb-2">Hey, {firstName}</h2>
        ...
        <div className="grid grid-cols-2 lg:grid-cols-4 gap-3">
          ...
          <StatCard icon={Lock}
            value={codes.filter(c => c.status === "static").length}
            label="Static Codes" color="#8B5CF6" />
        </div>
        ...
        <div className="grid lg:grid-cols-2 gap-4">
                  ...
                  return (
                    <div key={code.id as string}
                      ...
                      <div className="flex-1 min-w-0">
                        <div className="text-sm font-medium text-[#F0F4F8] truncate">{code.name as string || "—"}</div>
                        <div className="text-[11px] text-[#4A5568]">{(code.scans as number) || 0} scans</div>
                      </div>
          ...
          <div className="bg-[#0F1520] border border-[rgba(255,255,255,0.07)] rounded-xl p-5">
            ...
            {loading ? <div className="h-28 shimmer rounded-lg" /> : <BarChart codes={codes} />}
          </div>
        </div>
